...
function showTip() {
	...
	if (currentRow >= 2) {
		...
		wordtrim = word.trim();
		if (currentRow === 2) {
			tipsy.textContent = wordtrim.slice(0, 1) + "*".repeat(word.length - 2);
		} else if (currentRow === 3) {
			tipsy.textContent = wordtrim.slice(0, 2) + "*".repeat(word.length - 3);
		} else if (currentRow === 4) {
			tipsy.textContent = wordtrim.slice(0, 3) + "*".repeat(word.length - 4);
		} else if (currentRow === 5) {
			tipsy.textContent = wordtrim.slice(0, 4) + "*".repeat(word.length - 5);
		} else {
			tipsy.textContent = wordtrim;
		}
		...
		playNoTipSound();
		tipsy.textContent = 'لەدوای دوو هەوڵدان، دەتوانی بیرۆکەیەک لەسەر وشەکە وەربگری چەند پیتێکی یەکەمی وشەکە دەبینیت بەپێی ژمارەی هەوڵدانەکانت';
		tip.style.display = 'block';
		myDiv.classList.add('disabled-div');
		tip.style.pointerEvents = 'all';
		tip.append(tipsy);
...
control.innerHTML = '';
	...
	(async () => {
	  ...
	  if(userStatus === 50) {
		  showMessage("پیرۆزە! زیاتر لە پەنجا یاریت تەواو کرد!");
	  }
  
	  if(userStatus === 2) {
		  showMessage("سێیەم یاری! بەهیوای بەختێکی باش");
	  }
	  if(userStatus === 8) {
		  showMessage("هەشتەم یاری! زۆر سوپاس!");
	  }
  
	  if(userStatus === 100) {
		  showMessage("پیرۆزە!زیاتر لە سەد یاریت تەواو کرد!");
	  }
	  if(userStatus === 500) {
		  showMessage("پیرۆزە!زیاتر لە پێنچ سەد یاریت تەواو کرد!");
	  }
...
function setCookie(name, value, days) {
  ...
  // Function to set the chosen rating in a cookie
  function setChosenRating(rating) {
	setCookie("chosenRating", rating, 365);
  }
  
  // Function to get the chosen rating from the cookie
  function getChosenRating() {
	return getCookie("chosenRating");
  }
  ...
  function initializeToggleStates() {
	...
	var chosenRatingCookie = getChosenRating();
	if (chosenRatingCookie !== null) {
	  // Set the chosen rating
	  var ratingInputs = document.querySelectorAll('input[name="star-radio"]');
	  ratingInputs.forEach(function(input) {
		if (input.value === chosenRatingCookie) {
		  input.checked = true;
		}
	  });
	}
